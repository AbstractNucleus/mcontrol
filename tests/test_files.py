...
from mcontrol.file_safety import resolve_within
...
async def test_view_too_large_shows_size_and_mtime_caption(
    ...
    body = response.text
    assert "file-view__meta" in body
    # 6 MB threshold → MB-formatted size in the caption.
    assert " MB" in body
    assert "modified " in body
...
def test_humansize_filter_bands() -> None:
    """Issue #60: bytes for <1 KB, KB with 1 decimal for <1 MB, MB otherwise."""
    from mcontrol.templates import humansize

    assert humansize(0) == "0 B"
    assert humansize(512) == "512 B"
    assert humansize(1023) == "1023 B"
    assert humansize(1024) == "1.0 KB"
    assert humansize(4300) == "4.2 KB"
    assert humansize(1024 * 1024 - 1) == "1024.0 KB"
    assert humansize(1024 * 1024) == "1.0 MB"
    assert humansize(int(2.5 * 1024 * 1024)) == "2.5 MB"
